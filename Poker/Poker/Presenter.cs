    ...
    class Presenter
    {
        ...
        private void View_PassButtonClicked()
        {
            if (view.DiceResources.Contains(view.PassButton.Image))
            {
                ...
                if (view.Game.Rounds == 0) { MessageBox.Show("Please, select category from the right.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information); }
            }
        ...
        private void cellClicked(object sender, MouseEventArgs e)
        {
            ...
            if (view.Table.GetColumn(cell) == 1 && view.Table.GetRow(cell) > 0 && view.Table.GetRow(cell) < 14 && int.Parse(cell.Text) == -1)
            {
                ...
                else { MessageBox.Show("Finish your turn first!", "Wrong move", MessageBoxButtons.OK, MessageBoxIcon.Exclamation); }
            }
            else { MessageBox.Show("It has been already used!", "Wrong move", MessageBoxButtons.OK, MessageBoxIcon.Exclamation); }
