...
namespace Poker
{
    ...
    public void ChosenCategory(int row)
    {
        switch (row)
        {
            ...
            case 5:
            case 6:
                Score = categories.SumOfSameDices(FinalRolls, row);
                break;
            case 7:
                Score = categories.ThreeOfAKind(FinalRolls);
                break;
            case 8:
                Score = categories.ThreeOfAKind(FinalRolls);
                break;
            case 9:
                Score = categories.FullHouse(FinalRolls);
                break;
            case 10:
                Score = categories.SmallStraight(FinalRolls);
                break;
            case 11:
                Score = categories.LargeStraight(FinalRolls);
                break;
            case 12:
                Score = categories.Yahtzee(FinalRolls);
                break;
            case 13:
                Score = categories.Chance(FinalRolls);
                break;
            default:
                MessageBox.Show("Ups! Something went wrong.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                break;
        }
        TotalScore += Score;
    }
    ...
    public void End()
    {
        if (Rounds == 13)
        {
            MessageBox.Show($"Well Done! You got {TotalScore} points! Compare points", "Endgame", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        else { MessageBox.Show("This game shouldn't have end yet.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); }
    }
    #endregion
